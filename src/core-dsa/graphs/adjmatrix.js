export default class GraphAdjacencyMatrix {
    ...
    addEdgeDirected(v1, v2) {
        ...
        // since its directed
        this.adjacencyMatrix[v1][v2] = 1
        console.log("vertex", this.adjacencyMatrix)
    }
    bfs(startVertex) {
        ...
        while (queue.length > 0) {
            ...
            /**
             * Index of the current row in the adjacency-matrix iteration.
             *
             * Used as the loop counter to iterate from 0 up to (but not including)
             * the total number of vertices, accessing each row of the matrix.
             *
             * @type {number}
             * @private
             */
            for(let cell=0; cell<this.vertices; cell++){
                console.log("matrx", current, cell)
                if(this.adjacencyMatrix[current][cell]==1 && !visited.has(cell)){
                    visited.add(cell)
                    queue.push(cell)
                }
            }
